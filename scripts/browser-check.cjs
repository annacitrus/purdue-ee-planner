...
(async()=>{
 ...
 for(const url of ['http://localhost:3000','file:///'+path.resolve('index.html').replaceAll('\\','/')]){
  ...
  const hoverCheck=await send('Runtime.evaluate',{expression:`(() => {
   ...
   const calc=()=>card('Calculus-2');
   const originalMeta=calc().querySelector('.course-meta').textContent;
   const originalTotal=document.querySelector('#total-credits').textContent;
   calc().click();
   document.querySelector('#course-credits').value='4';
   document.querySelector('#save-note').click();
   assert(calc().querySelector('.course-meta').textContent.trim()==='4 credits','Chosen credits on card');
   const saved=JSON.parse(localStorage.getItem('purdue-ee-plan-v2'));
   assert(saved.creditHours[calc().dataset.key]===4,'Credits saved');
   assert(document.querySelector('#total-credits').textContent!==originalTotal || originalMeta.trim()==='4 credits','Total updated');
   calc().click();
   assert(document.querySelector('#course-credits').value==='4','Credits restored in details');
   document.querySelector('#course-credits').value='-1';
   document.querySelector('#save-note').click();
   assert(document.querySelector('#details').open,'Invalid credits rejected');
   document.querySelector('#details').close();
   document.querySelector('#undo').click();
   assert(calc().querySelector('.course-meta').textContent===originalMeta,'Undo restores credits');
   assert(document.querySelector('#total-credits').textContent===originalTotal,'Undo restores totals');
   return 'Hover, keyboard, colors, cleanup, details and credit editing passed';
  })()`,returnByValue:true});
